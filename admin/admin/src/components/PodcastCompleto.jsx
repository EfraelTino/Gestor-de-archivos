...
export const PodcastCompleto = ({ urlFiles, setUrlFiles, idpocast }) => {
  ...
  const handleUpdate = async (id, newValue) => {
    ...
    const updatedFiles = urlFiles.map((file) =>
  ...
  const handleAddItem = async (e) => {
    ...
    const newItem = {
      id:
        urlFiles.length > 0
          ? Math.max(...urlFiles.map((item) => item.id)) + 1
          : 1,
      files: nuevo,
      id_podcast: urlFiles[0]?.id_podcast || 5,
      tipo: 0,
    };
    const formdata = new FormData();
    ...
    try {
      ...
      toast.success(message);
      setUrlFiles((prevFiles) => [...prevFiles, newItem]);
      setNuevo("");
      setAdd(false);
    } catch (error) {
      console.error("Add item error: ", error); // Debug
      toast.error("Error inesperado, intente más tarde");
    }
  };
  ...
  const handleRemove = async (id) => {
    ...
    try {
      ...
      toast.success(message);
      setUrlFiles((prevFiles) => prevFiles.filter((item) => item.id !== id));
    } catch (error) {
      console.error("Remove error: ", error); // Debug
      toast.error("Error inesperado, intente más tarde");
    }
  };
